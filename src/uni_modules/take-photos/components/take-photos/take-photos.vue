	...
	<view class="info">
		<view class="main main_no_idcard">
			<template v-if="!imgSrc">
				<h5-cameral :facing-mode="constraints.video.facingMode" :photos-border="props.photosBorder"
					:photos-face-border="props.photosFaceBorder">
				</h5-cameral>

				<slot name="take-button">
					<tak-button v-if="!imgSrc" :rout-cameral-type="routCameralType" :cameral-type="cameralType"
						@handle-click-take="handleClickTake" @handle-click-device="handleClickDevice"></tak-button>
				</slot>
...
<script lang="ts" setup>
	...
	const initPhotos = async () => {
		...
		if (props.videoWidth && Object.keys(props.videoWidth).length) {
			constraints.video.width = props.videoWidth

			if (props.videoHeight && Object.keys(props.videoWidth).length) {
				constraints.video.height = props.videoHeight
			}
	...
	defineExpose({
		initPhotos,
		takePhots: handleClickTake,
		toggle: handleClickDevice,
	})
</script>
<style>
	.info {
