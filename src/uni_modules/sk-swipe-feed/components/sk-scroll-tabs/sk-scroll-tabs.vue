...
/** 首测失败后的补测延时（ms），需大于 REFRESH_DEBOUNCE + 一次测量往返 */
const MEASURE_RETRY = 150
...
const expose: SkScrollTabsExpose = { refresh, scrollToIndex }
defineExpose(expose)

// ==================== 监听与生命周期 ====================

// tabs 数据变更自动重新测量：deep 覆盖原地修改 item 的场景（如改 name/badge 使标签变宽）
watch(() => props.tabs, () => refresh(), { deep: true })
...
onMounted(() => {
  refresh()
  // 首测可能因布局未就绪拿不到尺寸（小程序端常见），此时 ready 仍为 false、滑块不渲染；
  // 延后补测一次（有界，不轮询），与 sk-swipe-page 的宽度补测策略一致
  retryTimer = setTimeout(() => {
    retryTimer = null
    if (!state.ready && props.tabs.length) refresh()
  }, MEASURE_RETRY)
  // 窗口尺寸变化（旋转/分屏）后容器宽与标签布局都可能陈旧，重测
